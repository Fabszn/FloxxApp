...
import org.floxx.env.service.{hitService, securityService}
...
object AppLoader {
  ...
  object appEnv{
    ...
    type AppEnvironment = Clock with Blocking with Has[Configuration]
      with Has[hitService.HitService] with Has[securityService.SecurityService]
    val appEnvironnement = Clock.live ++ Blocking.live ++ config.layer ++ hitService.layer ++ securityService.layer
  }
